#include<bits/stdc++.h>
using namespace std;
const int N=40,T=100,Allsum=20,MAXN=1000;
...
struct gene{
	...
	void calc(){
		...
		while (i<sum&&!(s==end)){
			...
			if (next.legal()){
				s=next,steps++;
				if (v.Mandis>end.dis(s))v.Mandis=end.dis(s),v.steps=steps;
			}
			i++;
		}
	}
...
}group[(Allsum+1)*(Allsum+1)],ans,help[Allsum+10];
void init(){
...
void calc(){
	...
	if (group[1].v<ans.v)ans=group[1];
